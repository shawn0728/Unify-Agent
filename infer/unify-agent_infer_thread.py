...
"""
Multi-turn threaded inference script for Unify-Agent.
...
def detect_image_format_from_bytes(img_bytes):
    """Detect real image format from raw bytes."""
    try:
        ...
        # PNG: 89 50 4E 47 0D 0A 1A 0A
            ...
            return 'image/png'
        # GIF: GIF87a or GIF89a - unsupported, return None
        elif len(img_bytes) >= 6:
            if img_bytes[:6] == b'GIF87a' or img_bytes[:6] == b'GIF89a':
                return None
        # WebP: RIFF...WEBP - unsupported, return None
        if len(img_bytes) >= 12:
...
def judge_image_quality(image_path, ip_name):
    """
    Use the reasoning API to judge image quality.
    
    Args:
        image_path: Path to the image.
        ip_name: IP name, used to assess whether the image correctly depicts this IP.
    
    Returns:
        dict with score, reason, is_text_heavy, has_watermark.
              On failure returns {"score": -1, "reason": "error message", ...}
    """
    ...
    if result.get("success") and result.get("text"):
        try:
            ...
            try:
                ...
                # If direct parse fails, try extracting a JSON block
...
def extract_recaption_content(text: str) -> str:
    ...
    # Try matching <recaption>...</recaption>
    match = re.search(r'<recaption>(.*?)</recaption>', text, re.DOTALL | re.IGNORECASE)
    if match:
        return match.group(1).strip()
    
    # If no closing tag, try matching everything after <recaption>
...
def normalize_recaption_text(text: str, language: str = 'zh') -> str:
    """Normalize recaption text: unify tag casing (<Think>-><think>, <Instruction>-><recaption>)."""
    if not text:
        return text
    normalized = text
...
def load_prompt_template(country: str = None) -> str:
    """Load the recaption system prompt based on country."""
    if country == "中国":
...
def download_image_to_bytes(image_url):
    """Download an image and return raw bytes (with retries)."""
    # Set up proxy
    proxies = {
...
def execute_text_search(parameters, use_summary=True):
    """Execute text_search tool, with optional summarization via reasoning API."""
    ...
    try:
        ...
        print(f"  Calling Serper API: q='{query}', hl='{lang}'")
        
        # Serper API call with retries
        serper_data = None
        ...
        # Check and use answerBox if present
        ...
        # Use Jina API to read content and summarize via reasoning API
        ...
        for idx, result in enumerate(organic_results[:top_k]):
            ...
            # Try reading full content via Jina API
            jina_success = False
            content = ""
            ...
            # If Jina succeeded, summarize via reasoning API
...
[Your concise summary here - 2-3 sentences maximum]
                ...
                # Call reasoning API for summary
                ...
                if gemini_result.get("success") and gemini_result.get("text"):
                    ...
                    if full_response:
                        ...
                        if response_match:
                            ...
                            # If no <response> tag found, use full response
                ...
                # If summarization failed, use first 500 chars of raw content
                results_summary.append(f"[{len(results_summary)+1}] {title}\n{content[:500]}...")
                text_search_debug["summary_items"].append({
                    "title": title,
                    "url": url,
            ...
            # If Jina read failed, use snippet as fallback
...
def execute_search_image(parameters):
    ...
    for attempt in range(MAX_RETRIES + 1):
        try:
            ...
            # Format image info (exclude Image URL, Source, Link to avoid leaking into dialogue)
            images_info = []
...
def download_and_judge_search_images(search_result, ip_name, ip_intermediate_dir):
    """Download searched images and judge quality, returning best image paths and scores.

    Args:
        search_result: Raw API response from execute_search_image (contains images list).
        ip_name: IP name for quality assessment.
...
- Image search: <tool_call>{{"name": "search_image", "arguments": {{"q": "image query", "hl": "zh", "num": 10}}}}</tool_call>
...
def build_stage3_prompt(
    ...
    """Build the Stage 3 prompt (recaption generation).
